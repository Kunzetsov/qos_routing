...
class NetworkDelayDetector(app_manager.RyuApp):
    ...
    def _detector(self):
        ...
        while CONF.get('weight') == 'delay':
            ...
            # print('here it is !!!')
            self.network_route_detector.handle_delay_update(self.delay_graph)
            # hub.sleep(settings.DELAY_DETECTING_PERIOD)
    ...
    def _send_echo_request(self):
        ...
        for datapath in self.datapaths.values():
            ...
            echo_req = parser.OFPEchoRequest(datapath, data="%.12f".encode('ascii') % time.time())
    ...
    def create_delay_graph(self):
        for src in self.awareness.graph:
            for dst in self.awareness.graph[src]:
                ...
                if self.delay_graph.get(src) is None:
                    self.delay_graph.setdefault(src, {})
                self.delay_graph[src][dst] = delay

                # self.logger.info("%s<-->%s : %s" % (src, dst, delay))

    def show_delay_statis(self):
        if settings.TOSHOW and self.awareness is not None:
            self.logger.info("\nsrc   dst      delay")
            self.logger.info("---------------------------")
            for src in self.awareness.graph:
                for dst in self.awareness.graph[src]:
