...
class NetworkMonitor(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.network_route_detector = lookup_service_brick('network_route_detector')
        self.graph = None
    ...
    def _monitor(self):
        ...
        while CONF.get('weight') == 'bw':
            ...
            for dp in self.datapaths.values():
                ...
                self.capabilities = None
                self.best_paths = None
            hub.sleep(settings.METRICS_DISCOVERING_PERIOD)
    ...
    def _save_bw_graph(self):
        ...
        while CONF.get('weight') == 'bw':
            ...
            bw_graph = self.create_bw_only_graph(self.graph)
            self.logger.debug("save_freebandwidth")
            self.network_route_detector.handle_bandwidth_update(bw_graph)
            # print('herere')
            hub.sleep(settings.METRICS_DISCOVERING_PERIOD)
        ...
        for src in paths:
            for dst in paths[src]:
                if src == dst:
                    ...
                    capabilities.setdefault(src, {src: settings.MAX_CAPACITY})
                    capabilities[src][src] = settings.MAX_CAPACITY
                    continue
                max_bw_of_paths = 0
                best_path = paths[src][dst][0]
                for path in paths[src][dst]:
                    min_bw = settings.MAX_CAPACITY
    ...
    def create_bw_only_graph(self, graph):
        # for src, dist_list in list(graph.nodes):
        bw_only = {}
        for src, dst, data in graph.edges.data():
            if bw_only.get(src) is None:
                bw_only.setdefault(src, {})

            bw_only[src][dst] = data.get('bandwidth', 0)
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for stat in sorted([flow for flow in body if flow.priority == 1],
            ...
            if len(stat.instructions) != 0 \
                    and len(stat.instructions[0].actions) != 0:
                ...
                # Get flow's speed.
                pre = 0
                period = settings.METRICS_DISCOVERING_PERIOD
    ...
    def _port_stats_reply_handler(self, ev):
        ...
        for stat in sorted(body, key=attrgetter('port_no')):
            ...
            if port_no != ofproto_v1_3.OFPP_LOCAL:
                ...
                self.network_route_detector.handle_speed_update(key, self.port_speed[key][-1])
        # ev.set()
